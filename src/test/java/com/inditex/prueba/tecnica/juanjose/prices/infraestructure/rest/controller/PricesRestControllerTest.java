...
import com.inditex.prueba.tecnica.juanjose.prices.api.rest.controller.PricesRestController;
...
class PricesRestControllerTest {
    ...
    @MockBean private GetPricesUseCase getPricesUseCase;

    private static final String CONTROLLER_PATH = "/v1/prices";
    ...
    void testGetPrices_Success(PriceTestData testData) throws Exception {
        ...
        // Mock del UseCase
        when(getPricesUseCase.getPricesForMarkAndProductByDate(
                        testData.getBrandId(),
                        testData.getProductId(),
                        testData.getAskDate().atZone(ZoneId.systemDefault()).toOffsetDateTime()))
                .thenReturn(testData.getExpectedPrice());

        // petición HTTP simulada
        mockMvc.perform(
                        MockMvcRequestBuilders.get(CONTROLLER_PATH)
                                .param("brandId", String.valueOf(testData.getBrandId()))
                                .param("productId", String.valueOf(testData.getProductId()))
                                .param("askDate", testData.getAskDate().toString())
                                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.brandId").value(testData.getBrandId()))
    ...
    @Test
    @DisplayName(
            "Test getPrices - Caso de no encontrado (PriceNotFoundException): Devolver 404 Not Found")
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        String.format(
                                                "No se encontró ningún precio para BrandId: %s, ProductId: %s y Fecha: %s.",
                                                brandId, productId, askDate)));
    }

    @Test
    @DisplayName(
            "Test getPrices - Caso de parámetro brandId mal formado (InvalidBrandIdException): Devolver 400 Bad Request")
    ...
    @Test
    @DisplayName(
            "Test getPrices - Caso de parámetro brandId mal formado (InvalidProductIdException): Devolver 400 Bad Request")
    ...
    @Test
    @DisplayName(
            "Test getPrices - Caso de parámetro obligatorio productId faltante: Devolver 400 Bad Request")
                ...
                .andExpect(
                        jsonPath(
                                ...
                                Matchers.containsString(
                                        "El parámetro 'productId' es obligatorio.")));
    }

    @Test
    @DisplayName(
            "Test getPrices - Caso de no encontrado (NoSuchElementException): Devolver 404 Not Found")
    void testGetPrices_NotFoundException() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "No se encontró ningún precio para BrandId: "
                                                + brandId
                                                + ", ProductId: "
                                                + productId
                                                + " y Fecha: "
